# -*- coding: utf-8 -*-
"""
Created on Tue Oct 10 14:17:09 2023

@author: Mieszko Ferens

Script to run an experiment for modelling an XOR Arbiter PUF integrated into a
QKD system while attemping to guess the PUF response through its bias.
"""
...
def main():
    ...
    # Bits guessed by Eve to obtain responses
    r_guess = np.random.rand(args.batch_size) < 0.7
    ...
    while(batch < n_batches):
        ...
        # Bits guessed by Eve to obtain responses
        r_guess = np.random.rand((batch+1)*args.batch_size) < 0.7
        
        # Training set generated by Eve with only the accurate responses with
        # high probability
        idx = np.where(r_guess == (response_guess.flatten() == 1))[0]
        train_crps = ChallengeResponseSet(
    ...
    filepath = Path(
        args.outdir + "out_PUQKD_bias_" + str(args.k) + "XOR.csv")
    if(filepath.is_file()):
